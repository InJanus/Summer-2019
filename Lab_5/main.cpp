...
int main(){
    ...
    // int *mysorteditemsbubble = bubbleSort(myArray, size);
    // for(int i = 0; i < size; i++){
    ...
    //We can add a feature that if the size is less we use a "delete"
    //So then we shave the array down to the correct size
    //Task 2 std::cout << "Delta t2-t1: " << std::chrono::duration_cast<std::chrono::nanoseconds>(t2 -t1).count()<< " nanoseconds" << std::endl;
    auto t1 = Clock::now();
